          ...
          <div className="flex items-center flex-1 md:flex-none justify-end gap-2 md:gap-6 min-w-0">
              ...
              <form onSubmit={handleSearch} className="group w-full">
                <div className="relative flex items-center bg-white/10 border border-transparent focus-within:border-white/20 rounded-full transition-all duration-300 w-full md:w-64 overflow-hidden h-9 md:h-10">
                    ...
                    <input
                        ...
                        className="bg-transparent text-white text-xs md:text-sm focus:outline-none placeholder-gray-400 w-full h-full pl-9 md:pl-10 pr-4"
                    />
                </div>
              </form>
              
              {showSuggestions && suggestions.length > 0 && (
                  <div className="absolute top-full right-0 mt-2 w-full md:w-80 bg-surface/95 backdrop-blur-xl border border-white/10 rounded-lg shadow-2xl overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200 max-h-[60vh] md:max-h-[80vh] overflow-y-auto">
                      {suggestions.map((group, gIdx) => (
                          <div key={gIdx}>
                              ...
                              {group.items.map((item) => (
                                ...
                                >
                                    <div className="w-8 h-12 md:w-10 md:h-14 shrink-0 rounded bg-gray-800 overflow-hidden mr-3 flex items-center justify-center">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <h4 className="text-xs md:text-sm font-medium text-white truncate">{item.title || item.name}</h4>
                                        <div className="flex items-center text-[10px] md:text-xs text-gray-400 mt-1">
                                            {item.vote_average > 0 && (
            ...
            {installPrompt && (
                <button 
                    ...
                    className="hidden md:flex items-center gap-2 bg-white/10 hover:bg-white/20 text-white px-3 py-1.5 rounded-full text-xs font-medium transition-colors"
